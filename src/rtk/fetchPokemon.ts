import { Pokemon, PokemonDetail, PokemonSpecies } from '../types';
...
// 1세대 포켓몬 ID 배열 (1~151)
const POKEMON_IDS: number[] = Array.from({ length: 151 }, (_, i) => i + 1);
 ...
 * @returns 가공된 포켓몬 정보
 */
async function fetchOnePokemon(id: number): Promise<Pokemon> {
  ...
  const pokemon: PokemonDetail = await pokemonRes.json();
